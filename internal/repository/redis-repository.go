...
const payButtonStateKey = "pay_button:enabled"

// GetPayButtonState - төлем батырмасының күйін алу
func (r *RedisRepository) GetPayButtonState(ctx context.Context) (bool, error) {
	val, err := r.client.Get(ctx, payButtonStateKey).Result()
	if err == redis.Nil {
		// егер ключ жоқ болса, әдепкі бойынша қосылған
		return true, nil
	}
	if err != nil {
		return true, err
...
// SetPayButtonState - төлем батырмасының күйін орнату
func (r *RedisRepository) SetPayButtonState(ctx context.Context, enabled bool) error {
	val := "0"
	if enabled {
		val = "1"
	}
	return r.client.Set(ctx, payButtonStateKey, val, 0).Err()
}
